...
- 若任务由 Issue 发起，PR body 末尾必须追加 `Ref #<CNB_ISSUE_IID>` (如 `Ref #11`) —— 遗漏会导致 Issue 中看不到关联的 PR；
- `pr-body` 使用单引号包裹传参，可减少不必要的 shell 转义，比如：可直接用**真换行**拼接，而不是 `\n` 字面量。
